"""
Copy Budgets / Financial Forecast data from production PostgreSQL to local SQLite.

Copies: financial_forecast_options, financial_forecast_mappings,
financial_forecast_mapping_sources, financial_forecast_lines.

Does not copy users or other app data. Nulls updated_by_user_id to avoid FK issues.
...
Usage:
  ...
  2. In PowerShell (from Farm-Dashboard-Web):
     ...
     py scripts/sync_financial_forecasts_from_production.py --dry-run
     py scripts/sync_financial_forecasts_from_production.py --replace --yes
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Copy Financial Forecast / Budgets tables from production into local SQLite."
    )
    parser.add_argument(
        "--source-url",
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Delete existing financial forecast data in the target before copying",
    )
    parser.add_argument(
        "--yes",
        action="store_true",
    ...
    if args.replace and not args.yes and not args.dry_run:
        print(
            "ERROR: Pass --yes with --replace to confirm overwriting local financial forecast data."
        )
        sys.exit(1)
    ...
    try:
        ...
        try:
            if args.replace:
                print("Clearing local financial forecast tables…")
